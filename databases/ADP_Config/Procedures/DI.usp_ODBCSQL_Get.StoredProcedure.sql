/****** Object:  StoredProcedure [DI].[usp_ODBCSQL_Get]    Script Date: 1/12/2020 11:43:32 PM ******/
DROP PROCEDURE IF EXISTS [DI].[usp_ODBCSQL_Get]
GO
/****** Object:  StoredProcedure [DI].[usp_ODBCSQL_Get]    Script Date: 1/12/2020 11:43:57 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE    PROCEDURE [DI].[usp_ODBCSQL_Get] (@TaskInstanceID INT)
...
BEGIN TRY
	...
	-- Check the ODBC connection type

	SELECT  
		@ODBCConnectionType = CP.ConnectionPropertyValue 
	FROM
		DI.Task T
	...
	WHERE
		T.TaskID = @TaskID
		AND CPT.ConnectionPropertyTypeName = 'ODBC Connection Type'
		AND CP.DeletedIndicator = 0

	-- Set the column delimitier. Use a ` for MySQL as it doesn't support double quotes

	IF ISNULL(@ODBCConnectionType, '') = 'MySQL'
