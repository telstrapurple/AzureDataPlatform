/****** Object:  StoredProcedure [DI].[usp_SchemaMapping_Get]    Script Date: 1/12/2020 11:43:32 PM ******/
DROP PROCEDURE IF EXISTS [DI].[usp_SchemaMapping_Get]
GO
/****** Object:  StoredProcedure [DI].[usp_SchemaMapping_Get]    Script Date: 1/12/2020 11:43:57 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE   PROCEDURE [DI].[usp_SchemaMapping_Get] (@TaskID INT)
...
AS
	...
	BEGIN TRY
		...
		-- If it is a JSON or XML file load 
		IF EXISTS 
			( 
				SELECT *
				FROM 
				...
				WHERE 
					TaskID = @TaskID
					AND TP.TaskPropertyValue IN ('Json', 'XML')
			)
		BEGIN 
		...
		--	@cols = STRING_AGG('"' + SourceColumnName + '"', ',')
			@cols = STRING_AGG('"' + CAST(SourceColumnName AS VARCHAR(MAX)) + '"', ',')
