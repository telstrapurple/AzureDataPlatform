...
-- =============================================
-- Dynamic Stored Procedure that does a merge between the Target Table and Tempstage Table which gets passed in through the parameters
-- =============================================
...
ALTER PROCEDURE [ETL].[usp_StagingTable_Load] @TaskConfig [VARCHAR](MAX),@TaskSchema [VARCHAR](MAX) AS


ALTER PROC [ETL].[usp_StagingTable_Load] @TaskConfig [VARCHAR](MAX),@TaskSchema [VARCHAR](MAX) AS
...
--	BEGIN TRANSACTION
	...
	SET @HistoryTargetTable = CONCAT(LEFT(@TargetTable, LEN(@TargetTable) - 1 ), '_history]')
	...
	IF OBJECT_ID('tempdb..#TargetSchema') IS NOT NULL
	BEGIN
		DROP TABLE #TargetSchema
	END
...
SELECT 
	   ...
	   ,CASE
			...
			--Replaced Above
			WHEN TempTable.OriginalColumnName IS NULL AND NOT(@IncludeSQLDataLineage = 'Y' AND TargetTable.OriginalColumnName LIKE 'ADS_%')
			THEN 'Delete'
	...
	--	DROP TABLE IF EXISTS #HistoryTargetSchema
		...
		IF OBJECT_ID('tempdb..#HistoryTargetSchema') IS NOT NULL
		BEGIN
			DROP TABLE #HistoryTargetSchema
		END
		...
		SELECT 
		   ...
		   ,CASE
				...
				--Replaced Above
				WHEN HistoryTempTable.OriginalColumnName IS NULL AND NOT(HistoryTargetTable.OriginalColumnName LIKE 'ADS_%' OR HistoryTargetTable.OriginalColumnName LIKE '__$%')				
				THEN 'Delete'
				ELSE 'Update'
			END AS ColumnAction
	...
	--Replaced Above
	IF @IncludeSQLDataLineage = 'Y' AND NOT EXISTS(SELECT 1 FROM #TargetSchema TS WHERE TS.OriginalColumnName LIKE 'ADS_%')
